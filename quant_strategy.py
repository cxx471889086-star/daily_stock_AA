...
def build_card(stocks, time_str):
    ...
    elements.append({
        "tag": "markdown",
        "content": f"🟢 **BUY: {buy_count}** | ⚪ **HOLD: {hold_count}** | 平均涨跌: **{avg_change:+.2f}%**"
    })
    ...
    for s in valid:
        ...
        text = f"{emoji} **{s['code']} {s.get('name', '?')}**\n" \
               f"   **{s.get('signal', '?')}** · 现价 **{s.get('price', 0):.2f}** ({s.get('change', 0):+.2f}%)\n" \
               f"   振幅 {s.get('amplitude', 0):.2f}% · 成交额 {amount_yi:.2f}亿"
    ...
    elements.append({
        "tag": "markdown",
        "content": f"<font color='gray'>更新于 {time_str} · 每 10 分钟自动推送</font>"
    })

    if buy_count > hold_count:
        template = "green"
    elif hold_count > buy_count:
        template = "orange"
    else:
        template = "blue"
